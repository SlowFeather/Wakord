# WakeUp · 本地中文语音唤醒（唤醒词：小元）

一个可**本地训练 + 常驻监听**的中文语音唤醒词项目。核心特性：

- 🟢 **随时可控的唤醒服务**：常驻后台，监听到唤醒词「小元」即回调；可被其他程序随时 **开始 / 停止** 监听。
- 🔌 **跨语言控制接口**：基于本机 TCP + JSON-lines 协议，任何语言的程序都能控制服务、接收「是否唤醒」消息。
- 🔋 **智能省电**：用极廉价的 VAD（人声检测）门控，**静默时不跑唤醒词模型**；停止监听时直接**释放麦克风**。
- 🈶 **中文且不损失精度**：用中文 TTS 合成正样本 + openWakeWord 语言无关的特征提取器训练自定义分类器（详见下文）。
- 🧩 **工程化结构**：按数据 / 训练 / 服务分层拆分，便于多人协作与提交 GitHub。
